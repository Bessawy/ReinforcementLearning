...
def train(agent, env, train_episodes, early_stop=True, render=False,
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            #print("AMR", reward)
            #print("Emad", observation)
            # TODO: Task 1 - change the reward function
            #reward = new_reward(observation, 1)
...
#cummulative reward of 1000 can be observed
...
#totalreward cannot reach 1000 at position not equal 0
def new_reward(state, position):
    ...
    #print(state[0])
    return reward
#speed move and change in direction
...
def main(args):
    ...
    if args.test is None:
        ...
        # Plot rewards
        sns.lineplot(x="train episode", y="reward", data=training_history)
        sns.lineplot(x="train episode", y="mean_reward", data=training_history)
        plt.legend(["Reward", "100-episode average"])
        plt.title("Reward history (%s)" % args.env)
        plt.show()
