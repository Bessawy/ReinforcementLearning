...
class Agent(object):
    def __init__(self, policy):
        self.train_device = "cpu"  # ""cuda" if torch.cuda.is_available() else "c torcdsasdapu"
    ...
    def episode_finished(self, episode_number):
        ...
        discounted = discount_rewards(all_rewards, self.gamma)
        discounted -= torch.mean(discounted)
        discounted /= torch.std(discounted)


        weighted_probs = all_actions * discounted
