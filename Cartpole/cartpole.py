...
def train(agent, env, train_episodes, early_stop=True, render=False,
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            #reward = new_reward_zero(observation)
            #reward = new_reward_position(observation, -1)
            reward = new_reward_speed(observation)
...
def test(agent, env, episodes, render=False):
    ...
    #boolen use to draw the first test episode only
    test = True
    observation_history, timestep_len = [], []
    for ep in range(episodes):
        ...
        while not done:
            ...
            #add the position and timesteps for first episode
            if test == True:
                observation_history.append(observation[1])
    ...
    #draw position of the first episode with time-step
    plt.plot(timestep_len, observation_history)
    plt.legend("Position from center")
    plt.xlabel('Time_steps')
    plt.ylabel('velocity')
    plt.title("Position of cart for the first test episode")
    plt.show()
...
#requires about 500 episode for ranges between +-0.5
#requires about 1000 episode for range between +-0.2
#Task1
def new_reward_zero(state):
    reward = 1
    x = state[0]
    #x > (-0.2) and x < (0.2)
    if x == 0 :
...
#Task2
def new_reward_position(state, position):
    reward = 1
    x = state[0]
    if x > (-0.2+position) and x < (0.2+position):
        reward = reward + 1
    #print(state[0])
    return reward
...
#Task3
def new_reward_speed(state):
    reward = 1
    v = state[1]
    #print(v)
    reward = reward + abs(v)*4
    return reward
